...
void Ramulator2::finish() {
  ...
  spdlog::info("{}: avg BW utilization {}% ({} reads, {} writes)", std_name,
               (tot_reads + tot_writes) * 100 * nbl / (2 * cycle_count),
...
void Ramulator2::cycle() {
  ...
  if (cycle_count > 0 && cycle_count % log_interval == 0) {
    if (memory_id == 0)
      spdlog::info("{}: BW utilization {}% ({} reads, {} writes)", std_name,
                   (num_reads + num_writes) * 100 * nbl / (2 * log_interval),
                   num_reads, num_writes);
    else
      spdlog::debug("{}: BW utilization {}% ({} reads, {} writes)", std_name,
                    (num_reads + num_writes) * 100 * nbl / (2 * log_interval),
                    num_reads, num_writes);
    num_reads = 0;
    num_writes = 0;
